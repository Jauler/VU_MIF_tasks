...
PID operuoja skaičiuodamas skirtumą tarp matuojamo kintamojo (vadinamo proceso kintamuoju) ir pateikto tikslo (ang.: \textit{setpoint}).
...
Čia $PID(t)$ yra proceso kintamasis, $e(t)$ -- proceso ir tikslo kintamųjų skirtumas.
$K_p$, $K_i$, $K_d$, PID algoritmo konstantos.
...
Šiuo atveju $\omega$ yra tai, kas yra valdoma PID algoritmo, norėdami tiesiškumo, ištraukiame šaknį iš PID išeities.
...
O PID paklaida yra skirtumas tarp matuojamo kintamojo (kampo nuo horizontalios pozicijos) ir piloto (arba autopiloto) nustatytos reikšmės.
...
PID valdiklio išeitis yra naudojama modifikuoti dabartiniam galios tikslui (ang.: \textit{setpoint}), jeigu tai yra PID valdiklis valdantis pasisukimą pagal $X$ ašį, tuomet antram varikliui galia ir didinama tiek, kokia yra PID išeitis, o trečiam atitinkamai mažinama.
Analogiškai veikia yra $Y$ ašis.
...
Pakeitus šį balansą galima priversti ketursraigtį suktis viena ar kita kryptimi, tačiau šia ašimi ketursraigčiai yra kur kas mažiau manevringesni negu $X$ ar $Y$ todėl valdymo algoritmas yra kur ka paprastesnis.
...
Taip ketursraigtis turi galimybę atpažinti ar nenutrūko ryšys su pilotu ar pilotojančia programa (žr.: TODO\_REFERENCE).
...
Pagrindinės valdymo elektronika sudaro GY-86 sensorių plokštė, su giroskopu ir akselerometrų, keturi vienetai greičio valdiklių, ir STM32F401 procesoriumi.
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=1.0\textwidth]{img/classDiagram.png}
