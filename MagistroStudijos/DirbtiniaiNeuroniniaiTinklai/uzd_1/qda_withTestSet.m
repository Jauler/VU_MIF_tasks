...
learn_set = [0, 3; 3, 3; 1, 2; 2, 2; 1, 1; 2, 1; 0, 0; 3, 0]'
...
# learn error estimation
learn_err = 0;
for i=1:1:length(learn_set)
	pos = learn_set(:, i);
	real_class = data(pos(1) + 1, pos(2) + 1);
	est_class = (pos - m(:, 2))' * inv(S_2) * (pos - m(:, 2)) - (pos - m(:, 1))' * inv(S_1) * (pos - m(:, 1)) + log(det(S_2) / det(S_1)) + 2 * log(P(1) / P(2));

	if real_class == 1 && est_class < 0
		learn_err = learn_err + 1;
	elseif real_class == 2 && est_class >= 0
		learn_err = learn_err + 1;
	endif
endfor

learn_err = learn_err / length(learn_set);
...
[2/9, -(1/9) * 2, -(4/3) * 2, -(1/9) * 2, 38/117, -(22/39) * 2, -(4/3) * 2, -(22/39) * 2, 251803469/90642045]
err * 100
learn_err * 100
